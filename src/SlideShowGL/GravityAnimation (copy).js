...
var vertexShaderSource = `		
	...
	mat4 genGravityTransform(int gravID){		
		float gravityRad = 0.1;
		
		vec3 arbVec0 = vec3(randoms);
		vec3 arbVec1 = vec3(randoms.z, randoms.x, randoms.z);
		
		vec3 arbVec0Uni = arbVec0 / length(arbVec0);
		vec3 arbVec1Uni = arbVec1 / length(arbVec1);
		
		vec3 orthoVec = cross(arbVec0Uni, arbVec1Uni);
		vec3 orthoVecUni = orthoVec / length(orthoVec);
		
		float rotations = 3.0;
		float rad = progress * rotations * PI * 2.0;
		
		mat4 GRM = rotationAroundAxis(orthoVecUni, rad);
