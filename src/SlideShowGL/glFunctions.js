...
class GlFunctionsInstantiator{
	...
	evalUniformLayoutLocation(name, data){
		...
		if(!data && this.uniforms[name]){
			this.uniforms[name].shaderLoc = shaderLoc;
		}else{
			this.uniforms[name] = {shaderLoc: shaderLoc, data: data};
		}
	...
	genAndBindVectorVBO(data, size, vboName, stride=0, offset=0){	
		...
		let layoutLoc = this.layoutLocations[vboName];
		if(layoutLoc < 0){
		...
		this.vboMeta[vboName] = shaderData;
		return shaderData;
	}
	...
	evalLayoutLocations(vboNames){
		...
		for(let vboName of vboNames){
			this.layoutLocations[vboName] = this.gl.getAttribLocation(shaderProgram, vboName);
		}
	}
...
export {GlFunctionsInstantiator};