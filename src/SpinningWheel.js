...
class RotatedDiv extends Component{
	render(){
		...
		let adjRotRad = (rotRad > Math.PI && rotRad < Math.PI*2) ? (rotRad + Math.PI) : rotRad;
		const rot = {
...
class SpinningWheel extends Component{
   ...
   setSpinningWheelSize(){
		...
		this.setState({spinninWheelSize: [sw.offsetWidth, sw.offsetHeight]});
	}
	render(){
		...
		return (
			<div className="SpinningWheel TextNotSelectable"
				...
				{this.props.tags.map( (tag,i)=>{
					return <RotatedDiv
							...
							fontSize={this.state.spinningWheelFontSize}
							xOffset={rad}
							yOffset={rad}
